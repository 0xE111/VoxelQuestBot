...
async def echo_handler(message: Message):
    ...
    if has(text, ['start', 'старт']):
        await answer_batch(message, [
            ...
            f'Для каждой загадки ответ - это какое-то слово или фраза, регистр не важен. Отвечать нужно только когда бот ждёт от вас ответа. Если застряли или ещё какой вопрос, то пишите мне на @{TELEGRAM_HELP_NICKNAME} - подскажу :winking_face:',
        ...
        await answer_batch(message, [
            ...
            'Привычным движением я включаю ноут. Пароль защитит от кражи ноута, но сильно я не заморачивался: кому надо - дешифрует любые пароли, если вы понимаете, о чём я. Поэтому пароль - прямо тут, на столе, в фоторамке:',
        ])
        ...
        await answer_batch(message, [
            ...
            'Включаю комп, линукс радуется мне, и я открываю ту самую картинку, которая на самом деле скрывает в себе кое-что. И да, я знаю, что использовать одни и те же пароли - плохо. Но тут расчет на то, что никто вообще не будет искать что-то в картинке.',
            'А в картинке-то спрятаны интересные слова...',
        ])
